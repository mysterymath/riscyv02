...
module riscyv02_regfile (
  ...
  generate
    ...
    for (gi = 0; gi < 8; gi = gi + 1) begin : gen_reg
      wire wen = w_we_r && !w_sel_r[3] && (w_sel_r[2:0] == gi[2:0]);
      wire gate = clk_n & wen;
  ...
  wire [15:0] regs_epc;
  wire wen_epc = w_we_r && w_sel_r[3];
  wire gate_epc = clk_n & wen_epc;

  generate
    for (bi = 0; bi < 16; bi = bi + 1) begin : gen_epc
      sg13g2_dlhrq_1 u_follower (
        .D(w_data_r[bi]), .GATE(gate_epc), .RESET_B(rst_n), .Q(regs_epc[bi])
      );
    end
  endgenerate
